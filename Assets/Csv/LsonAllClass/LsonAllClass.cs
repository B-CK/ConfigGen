...
namespace Csv.LsonAllClass
{
	public class LsonAllClass : CfgObject
	{
		...
		/// 类类型
		/// <summary>
		public Csv.LsonAllClass.LSingleClass VarClass;
		/// <summary>
		/// 字符串列表
		/// <summary>
		...
		/// Class列表
		/// <summary>
		public readonly List<Csv.LsonAllClass.LSingleClass> VarListClass = new List<Csv.LsonAllClass.LSingleClass>();
		/// <summary>
		/// Elem列表
		/// <summary>
		...
		/// 类类型字典
		/// <summary>
		public readonly Dictionary<string, Csv.LsonAllClass.LSingleClass> VarDictClass = new Dictionary<string, Csv.LsonAllClass.LSingleClass>();
		...
		public LsonAllClass(DataStream data)
		{
			...
			this.VarClass = (LsonAllClass.LSingleClass)data.GetObject(data.GetString());
			...
			for (int n = data.GetInt(); n-- > 0; )
			{
				this.VarListClass.Add((LsonAllClass.LSingleClass)data.GetObject(data.GetString()));
			}
			for (int n = data.GetInt(); n-- > 0; )
			{
			...
			for (int n = data.GetInt(); n-- > 0;)
			{
				...
				this.VarDictClass[k] = (LsonAllClass.LSingleClass)data.GetObject(data.GetString());
			}
		}
	}
}
