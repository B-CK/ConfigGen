...
namespace Csv.JLson
{
	public class LsonAllClass : CfgObject
	{
		...
		/// 类类型
		/// <summary>
		public Csv.JLson.LSingleClass VarClass;
		/// <summary>
		/// 字符串列表
		/// <summary>
		...
		/// Class列表
		/// <summary>
		public readonly List<Csv.JLson.LSingleClass> VarListClass = new List<Csv.JLson.LSingleClass>();
		/// <summary>
		/// Elem列表
		/// <summary>
		...
		/// 类类型字典
		/// <summary>
		public readonly Dictionary<string, Csv.JLson.LSingleClass> VarDictClass = new Dictionary<string, Csv.JLson.LSingleClass>();
		...
		public LsonAllClass(DataStream data)
		{
			...
			this.VarClass = (JLson.LSingleClass)data.GetObject(data.GetString());
			...
			for (int n = data.GetInt(); n-- > 0; )
			{
				this.VarListClass.Add((JLson.LSingleClass)data.GetObject(data.GetString()));
			}
			for (int n = data.GetInt(); n-- > 0; )
			{
			...
			for (int n = data.GetInt(); n-- > 0;)
			{
				...
				this.VarDictClass[k] = (JLson.LSingleClass)data.GetObject(data.GetString());
			}
		}
	}
}
