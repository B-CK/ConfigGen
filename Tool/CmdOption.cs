            ...
            for (int i = 0; i < args.Length; i++)
            {
                switch (args[i])
                {
                    case CONFIG_XML:
                        ...
                        Setting.Module = Path.Combine(Setting.ApplicationDir, NormalizePath(args[i]));
                        if (!File.Exists(Setting.Module))
                        {
                            Util.LogErrorFormat("[{0}]confgxml配置文件{1}不存在!", CONFIG_XML, Setting.Module);
                            _result &= false;
                        }
                        Setting.ExcelDir = Path.GetDirectoryName(Setting.Module);
                        break;
                    case DATA_DIR:
