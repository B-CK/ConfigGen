    ...
    public class ConstWrap
    {
        ...
        public void VerifyDefine()
        {
            if (!Util.MatchIdentifier(Name))
                Error("命名不合法:" + Name);
            if (!Setting.ConstTypes.Contains(_fullType) && !EnumWrap.IsEnum(_fullType))
            ...
            switch (FullType)
            {
                ...
                case Setting.FLOAT:
                    ...
                    break;
                default:
                    if (EnumWrap.IsEnum(_fullType) && !EnumWrap.Enums[_fullType].ContainItem(Value))
                        Error("值与类型不匹配:" + Name);
                    break;
            }
        ...
        {
            return string.Format("Const - Name:{0}\tType:{1}\tValue:{2}\tGroup:{3}", Name, FullType, Value, _group);
        }
        void Error(string msg)
        {
            string error = string.Format("常量字段信息异常!\n{0}Const:{1}({2})  {3}", _host, Name, FullType, msg);
