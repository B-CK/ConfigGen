...
namespace Tool
{
    ...
    public class Program
    {
        ...
        #region 协议方案
        const string protobuffers = "pb";
        const string flatbuffers = "flat";
        #endregion
        ...
        static void Usage()
        {
            ...
            Console.WriteLine($"\t\t{LAN}:[语言]\t生成指定语言代码,目前支持:pb,flat");
        ...
        static bool CheckCmdArgs()
        {
            ...
            else
            {
                ...
                {
                    string path = Path.Combine(Setting.CurrentDir, Util.NormalizePath(CmdDict[INPUT]));
                    if (File.Exists(path) && Directory.Exists(path))
                        Util.LogErrorFormat("{0}输入路径不存在!", module);
                }
        ...
        static void LoadDefine()
        {
            ...
            try
            {
                var moduleXml = Util.Deserialize(Setting.Input, typeof(ModuleXml)) as ModuleXml;
                ...
                string moduleDir = Path.GetDirectoryName(Setting.Input);
                string namespaceDir = $"{moduleDir}\\..\\Namespace";
                List<string> include = moduleXml.Imports;
                for (int i = 0; i < include.Count; i++)
                {
                    ///命名空间目录与模块目录并列
                    path = Path.Combine(namespaceDir, include[i] + ".xml");
