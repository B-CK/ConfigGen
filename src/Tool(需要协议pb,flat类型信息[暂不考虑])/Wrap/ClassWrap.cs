...
namespace Wrap
{
    ...
    public class ClassWrap
    {
        ...
        public HashSet<string> Chindren => _children;
        ...
        public ClassWrap(ClassXml des, string namespace0)
        {
            ...
            for (int i = 0; i < des.Fields.Count; i++)
            {
                ...
                string type = fieldXml.Value.IsEmpty() ? fieldXml.Type : $"{fieldXml.Type}:{fieldXml.Value}";
                var info = new FieldWrap(this, fieldXml.Name, type, fieldXml.Group, fieldXml.Desc, _groups);
                info.Init(fieldXml.Checker);
                Fields.Add(info);
            }
        }
