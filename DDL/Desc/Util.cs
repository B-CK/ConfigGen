...
namespace Desc
{
    ...
    public static class Util
    {
        ...
        #region 基础配置----所有路径相对模块文件路径:便于解析数据获取路径
        /// <summary>
        /// 工具所在目录
        /// </summary>
        ...
        {
            return GetRelativePath(path, DataDir);
        }
        //获取相对某个目录的相对路径
        public static string GetRelativePath(string absolutePath, string folder)
        {
            string path = absolutePath.ToStandardlize();
            bool nf = !File.Exists(path) && !Directory.Exists(path);
            bool nd = !Directory.Exists(folder.ToStandardlize());
            if (nf || nf)
            {
        ...
        public static string ToStandardlize(this string path)
        {
            return path.Replace("\\", "/");
        }
