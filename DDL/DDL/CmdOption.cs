            ...
            for (int i = 0; i < args.Length; i++)
            {
                switch (args[i])
                {
                    case CONFIG_XML:
                        ...
                        Setting.ModuleXml = Path.Combine(Setting.ApplicationDir, Util.NormalizePath(args[i]));
                        if (!File.Exists(Setting.ModuleXml))
                        {
                            Util.LogErrorFormat("[{0}]confgxml配置文件{1}不存在!", CONFIG_XML, Setting.ModuleXml);
                            _result &= false;
                        }
                        Setting.ConfigDir = Path.GetDirectoryName(Setting.ModuleXml);
                        break;
                    case DATA_DIR:
